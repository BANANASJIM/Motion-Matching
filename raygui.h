...
static void GuiTooltip(Rectangle controlRec)
{
    if (!guiLocked && guiTooltip && (guiTooltipPtr != NULL))
    {
        ...
        GuiPanel(Rectangle{ controlRec.x, controlRec.y + controlRec.height + 4, textSize.x + 16,static_cast<float>(GuiGetStyle(DEFAULT, TEXT_SIZE)) + 8.0f }, NULL);

        int textPadding = GuiGetStyle(LABEL, TEXT_PADDING);
        int textAlignment = GuiGetStyle(LABEL, TEXT_ALIGNMENT);
        GuiSetStyle(LABEL, TEXT_PADDING, 0);
        GuiSetStyle(LABEL, TEXT_ALIGNMENT, TEXT_ALIGN_CENTER);
        GuiLabel(Rectangle{ controlRec.x, controlRec.y + controlRec.height + 4, textSize.x + 16, static_cast<float>(GuiGetStyle(DEFAULT, TEXT_SIZE)) + 8.0f }, guiTooltipPtr);
